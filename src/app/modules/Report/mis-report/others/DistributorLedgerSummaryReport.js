...
export default function DistributorLedgerSummaryReport() {
    ...
    const generateReport = () => {
        ...
        const URL = `${process.env.REACT_APP_API_URL}/api/reports/distributor-ledger-summary-report` + queryParams;
        axios.get(URL, { responseType: 'blob' }).then(response => {
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            if (reportFormat === "PDF") {
                link.setAttribute('download', "DistributorLedgerSummary.pdf");
            }
            else {
                link.setAttribute('download', "DistributorLedgerSummary.xlsx");
            }
    ...
    const preview = () => {
        ...
        const dataURL = `${process.env.REACT_APP_API_URL}/api/reports/distributor-ledger-summary-report` + queryParams;
    ...
    return (
        <>
            <div>
                <MisReportBreadCrum menuTitle="Distributor Ledger Summary" />
            </div>
            <div>
                <Card>
                    <CardBody>
